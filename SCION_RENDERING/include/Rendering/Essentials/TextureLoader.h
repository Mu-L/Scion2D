...
class TextureLoader
{
  public:
	 ...
	 */
	static std::unique_ptr<Texture> Create( Texture::TextureType type, const std::string& texturePath,
											bool bTileset = false );
	static std::unique_ptr<Texture> Create( Texture::TextureType type, int width, int height,
													  bool bTileset = false );

	static std::unique_ptr<Texture> CreateFromMemory( const unsigned char* imageData, size_t length,
